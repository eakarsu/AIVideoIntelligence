...
// Multi-site federation with regional aggregation views
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, feature: 'multi-site-federation', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Multi-site federation with regional aggregation views. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'multi-site-federation', title: 'Multi-site federation with regional aggregation views', project: 'AIVideoIntelligence' });
});
