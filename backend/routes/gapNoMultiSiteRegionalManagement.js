...
// Feature: No multi-site/regional management
...
router.post('/run', async (req, res) => {
  ...
  try {
    ensureGapTable();
    const sys = 'You are an assistant for the "No multi-site/regional management" feature in project AIVideoIntelligence. Respond as strict JSON.';
    const user = `Feature request: No multi-site/regional management\n\nUser input JSON:\n` + JSON.stringify(input).slice(0, 4000) + '\n\nReturn JSON with summary, findings array, recommendations array.';
    const out = await callOpenRouter(sys, user);
    if (pool) {
      try { await pool.query('INSERT INTO gap_features(feature_slug, project, input, output) VALUES ($1,$2,$3,$4)', ['gap-no-multi-site-regional-management', 'AIVideoIntelligence', JSON.stringify(input), JSON.stringify(out)]); } catch (_) {}
    }
    res.json({ success: true, feature: 'gap-no-multi-site-regional-management', kind: 'gap_non_ai', result: out });
  } catch (err) {
...
router.get('/history', async (req, res) => {
  ...
  try {
    ensureGapTable();
    const { rows } = await pool.query('SELECT id, input, output, created_at FROM gap_features WHERE feature_slug = $1 ORDER BY created_at DESC LIMIT 50', ['gap-no-multi-site-regional-management']);
