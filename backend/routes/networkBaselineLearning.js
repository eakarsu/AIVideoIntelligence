...
// Network behavior baseline learning auto-detecting deviations
// Feature: network-baseline-learning
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, feature: 'network-baseline-learning', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Network behavior baseline learning auto-detecting deviations. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'network-baseline-learning', title: 'Network behavior baseline learning auto-detecting deviations', project: 'AIVideoIntelligence' });
});
