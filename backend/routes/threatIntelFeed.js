...
// Threat intelligence feed integration correlating anomalies with external IOCs
// Feature: threat-intel-feed
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, feature: 'threat-intel-feed', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Threat intelligence feed integration correlating anomalies with external IOCs. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'threat-intel-feed', title: 'Threat intelligence feed integration correlating anomalies with external IOCs', project: 'AIVideoIntelligence' });
});
