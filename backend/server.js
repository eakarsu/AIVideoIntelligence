...
app.post('/api/firmware/initiate', auth, async (req, res) => {
  try {
    ...
    const r = await pool.query(
      ...
      [device_id, device_name, property_name, current_version, target_version, 'admin@vigilance.ai']
    );
  ...
  try {
    ...
    for (const d of devList) {
      const r = await pool.query(
        ...
        [d.device_id, d.device_name, d.property_name, d.current_version, d.target_version, scheduled_at, 'admin@vigilance.ai']
      );
...
app.post('/api/ai/soc-copilot', auth, async (req, res) => {
  try {
    ...
    const systemMsg = `You are the Vigilance AI SOC (Security Operations Center) Copilot AI assistant. You help security operators monitor and manage building security across a portfolio of commercial properties.
...
app.post('/api/ai/predictive-maintenance', auth, async (req, res) => {
  try {
    ...
    const answer = await callAI([
      { role: 'system', content: 'You are a predictive maintenance AI for Vigilance AI. Analyze work order history and device status to predict failures and recommend preventive maintenance. Use markdown formatting.' },
...
app.post('/api/ai/firmware-risk-analyzer', auth, async (req, res) => {
  try {
    ...
    const answer = await callAI([
      { role: 'system', content: 'You are a firmware risk analyst for Vigilance AI. Analyze firmware versions against the catalog to identify CVE exposure and outdated devices. Review failed updates to explain root causes. Prioritize firmware rollout by criticality and property. Use markdown formatting with tables for rollout priority and clear risk ratings (Critical/High/Medium/Low).' },
...
app.post('/api/ai/device-copilot', auth, async (req, res) => {
  try {
    ...
    const systemMsg = `You are the Vigilance AI Device Intelligence Copilot. You help operators understand and manage all hardware infrastructure — devices, firmware, network topology, and health metrics.
...
app.post('/api/ai/incident-correlation', auth, async (req, res) => {
  try {
    const [incidents, events, access] = await Promise.all([
      ...
      pool.query('SELECT * FROM device_events ORDER BY created_at DESC LIMIT 50'),
      pool.query("SELECT * FROM access_events WHERE status = 'Denied' ORDER BY id DESC LIMIT 30"),
    ]);
    const answer = await callAI([
      { role: 'system', content: 'You are a security incident correlation engine for Vigilance AI. Analyze incidents, device events, and denied access events to find hidden connections, recurring patterns, and potential coordinated threats across properties. Group correlated events into clusters. Use markdown with headers, bullet points, and severity ratings.' },
      { role: 'user', content: `Correlate these security events and find hidden connections:\n\nINCIDENTS:\n${incidents.rows.map(i => `[${i.severity}] ${i.title} @ ${i.property_name} - ${i.status} - ${i.incident_type}`).join('\n')}\n\nDEVICE EVENTS:\n${events.rows.map(e => `[${e.severity}] ${e.event_type} - ${e.device_name} @ ${e.property_name}`).join('\n')}\n\nDENIED ACCESS:\n${access.rows.map(a => `${a.person_name} at ${a.door_name} @ ${a.property_name} (${a.method})`).join('\n')}` }
    ], 2500);
    res.json({ response: answer });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/ai/security-scorecard', auth, async (req, res) => {
  try {
    const [props, cams, incs, guards, zones, devices] = await Promise.all([
      pool.query('SELECT * FROM properties'),
      pool.query('SELECT property_name, status FROM cameras'),
      pool.query('SELECT property_name, severity, status FROM incidents'),
      pool.query('SELECT property_name, status FROM guards'),
      pool.query('SELECT property_name, access_level, alarm_enabled FROM zones'),
      pool.query('SELECT property_name, status FROM devices'),
    ]);
    const answer = await callAI([
      { role: 'system', content: 'You are a security posture analyst for Vigilance AI. Score each property on a 0-100 scale across dimensions: Camera Coverage, Incident Response, Guard Coverage, Zone Security, Device Health, and Overall. Identify weakest properties and recommend priority improvements. Output a professional scorecard with tables and letter grades (A-F).' },
      { role: 'user', content: `Generate a security posture scorecard:\n\nPROPERTIES:\n${props.rows.map(p => `${p.name} (${p.city}) - ${p.cameras_count} cams, ${p.access_points} access pts`).join('\n')}\n\nCAMERAS:\n${JSON.stringify(cams.rows.reduce((acc, c) => { acc[c.property_name] = acc[c.property_name] || { online: 0, offline: 0 }; c.status === 'Online' ? acc[c.property_name].online++ : acc[c.property_name].offline++; return acc; }, {}))}\n\nINCIDENTS:\n${JSON.stringify(incs.rows.reduce((acc, i) => { acc[i.property_name] = acc[i.property_name] || []; acc[i.property_name].push({ severity: i.severity, status: i.status }); return acc; }, {}))}\n\nGUARDS:\n${JSON.stringify(guards.rows.reduce((acc, g) => { acc[g.property_name] = acc[g.property_name] || { onDuty: 0, total: 0 }; acc[g.property_name].total++; if (g.status === 'On Duty') acc[g.property_name].onDuty++; return acc; }, {}))}\n\nDEVICES:\n${JSON.stringify(devices.rows.reduce((acc, d) => { acc[d.property_name] = acc[d.property_name] || { online: 0, offline: 0 }; d.status === 'Online' ? acc[d.property_name].online++ : acc[d.property_name].offline++; return acc; }, {}))}` }
    ], 3000);
    res.json({ response: answer });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/ai/shift-handoff', auth, async (req, res) => {
  try {
    const [incidents, events, access, guards, workOrders] = await Promise.all([
      ...
      pool.query('SELECT * FROM device_events ORDER BY created_at DESC LIMIT 20'),
      pool.query("SELECT * FROM access_events WHERE status = 'Denied' ORDER BY id DESC LIMIT 10"),
      pool.query('SELECT * FROM guards ORDER BY property_name'),
      pool.query("SELECT * FROM work_orders WHERE status != 'Completed' ORDER BY priority DESC LIMIT 10"),
    ]);
    const answer = await callAI([
      { role: 'system', content: 'You are a shift handoff report generator for Vigilance AI. Create a concise, actionable briefing for the incoming security shift. Include: critical items needing immediate attention, open incidents status, device issues, pending work orders, guard assignments, and key observations. Format as a professional shift handoff document with clear priorities.' },
      { role: 'user', content: `Generate a shift handoff briefing:\n\nOPEN INCIDENTS:\n${incidents.rows.map(i => `[${i.severity}] ${i.incident_number}: ${i.title} @ ${i.property_name} - Status: ${i.status}`).join('\n')}\n\nRECENT EVENTS:\n${events.rows.map(e => `[${e.severity}] ${e.event_type} - ${e.device_name} @ ${e.property_name}`).join('\n')}\n\nDENIED ACCESS:\n${access.rows.map(a => `${a.person_name} at ${a.door_name} @ ${a.property_name}`).join('\n')}\n\nGUARD ROSTER:\n${guards.rows.map(g => `${g.full_name} - ${g.property_name} - ${g.shift} shift - ${g.status}`).join('\n')}\n\nPENDING WORK ORDERS:\n${workOrders.rows.map(w => `[${w.priority}] ${w.work_order_number}: ${w.title} @ ${w.property_name} - ${w.status}`).join('\n')}` }
    ], 2500);
    res.json({ response: answer });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/ai/tenant-risk', auth, async (req, res) => {
  try {
    const [tenants, incidents, access, credentials] = await Promise.all([
      pool.query('SELECT * FROM tenants'),
      pool.query('SELECT property_name, severity, incident_type, status FROM incidents'),
      pool.query("SELECT property_name, status, person_name FROM access_events WHERE status = 'Denied'"),
      pool.query("SELECT property_name, department, status FROM credentials WHERE status != 'Active'"),
    ]);
    const answer = await callAI([
      { role: 'system', content: 'You are a tenant risk profiler for Vigilance AI. Analyze tenants based on their floor locations, nearby incidents, expired/revoked credentials, denied access patterns, and lease timelines. Assign risk levels and recommend security measures per tenant. Use markdown tables and risk ratings.' },
      { role: 'user', content: `Profile tenant risk:\n\nTENANTS:\n${tenants.rows.map(t => `${t.company_name} @ ${t.property_name} - Floor ${t.floor}, Suite ${t.suite} - ${t.employee_count} employees, ${t.badge_count} badges - Access: ${t.access_hours} - Lease ends: ${t.lease_end}`).join('\n')}\n\nINCIDENTS BY PROPERTY:\n${incidents.rows.map(i => `[${i.severity}] ${i.incident_type} @ ${i.property_name} - ${i.status}`).join('\n')}\n\nDENIED ACCESS:\n${access.rows.map(a => `${a.person_name} @ ${a.property_name}`).join('\n')}\n\nPROBLEM CREDENTIALS:\n${credentials.rows.map(c => `${c.department} @ ${c.property_name} - ${c.status}`).join('\n')}` }
    ], 2500);
    res.json({ response: answer });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/ai/camera-blindspot', auth, async (req, res) => {
  try {
    const [cameras, zones, props, incidents] = await Promise.all([
      ...
      pool.query('SELECT * FROM zones'),
      pool.query('SELECT * FROM properties'),
      pool.query('SELECT property_name, incident_type, severity FROM incidents'),
    ]);
    const answer = await callAI([
      { role: 'system', content: 'You are a camera coverage analyst for Vigilance AI. Analyze camera placement vs zone definitions, identify potential blind spots, gaps in coverage, and areas where incidents occur but camera coverage is weak. Recommend new camera placements with specific types (PTZ, Dome, Bullet, Thermal) and locations. Use markdown with tables showing coverage gaps.' },
      { role: 'user', content: `Analyze camera coverage and find blind spots:\n\nCAMERAS:\n${cameras.rows.map(c => `${c.name} @ ${c.property_name} - ${c.location_description} - Type: ${c.camera_type} - ${c.status} - ${c.resolution}`).join('\n')}\n\nZONES:\n${zones.rows.map(z => `${z.name} @ ${z.property_name} - Floor ${z.floor} - Type: ${z.zone_type} - Access: ${z.access_level} - Cameras: ${z.cameras_count} - Doors: ${z.doors_count} - Alarm: ${z.alarm_enabled}`).join('\n')}\n\nPROPERTIES:\n${props.rows.map(p => `${p.name} - ${p.floors} floors, ${p.cameras_count} cameras, ${p.access_points} access points`).join('\n')}\n\nINCIDENT LOCATIONS:\n${incidents.rows.map(i => `[${i.severity}] ${i.incident_type} @ ${i.property_name}`).join('\n')}` }
    ], 2500);
    res.json({ response: answer });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/ai/emergency-response', auth, async (req, res) => {
  try {
    ...
    const [props, guards, cameras, zones] = await Promise.all([
      pool.query('SELECT * FROM properties'),
      pool.query('SELECT * FROM guards'),
      pool.query('SELECT name, property_name, status, camera_type FROM cameras'),
      pool.query('SELECT name, property_name, zone_type, access_level, max_occupancy, alarm_enabled FROM zones'),
    ]);
    const answer = await callAI([
      { role: 'system', content: 'You are an emergency response planner for Vigilance AI. Given a scenario, create a detailed response plan including: immediate actions, guard deployment, camera priorities, zone lockdown sequence, communication chain, evacuation procedures, and recovery steps. Be specific with property names, guard names, and device assignments.' },
      { role: 'user', content: `Create emergency response plan for: ${scenario || 'Active security breach at the main lobby with unauthorized individuals attempting to access restricted floors'}\n\nPROPERTIES:\n${props.rows.map(p => `${p.name} (${p.city}) - ${p.floors} floors, ${p.cameras_count} cams, ${p.access_points} access pts`).join('\n')}\n\nGUARDS:\n${guards.rows.map(g => `${g.full_name} - ${g.property_name} - ${g.shift} (${g.status}) - Certs: ${g.certifications}`).join('\n')}\n\nCAMERAS:\n${cameras.rows.map(c => `${c.name} @ ${c.property_name} - ${c.camera_type} - ${c.status}`).join('\n')}\n\nZONES:\n${zones.rows.map(z => `${z.name} @ ${z.property_name} - ${z.zone_type} - Access: ${z.access_level} - Max Occ: ${z.max_occupancy} - Alarm: ${z.alarm_enabled}`).join('\n')}` }
    ], 3000);
    res.json({ response: answer });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/ai/work-order-optimizer', auth, async (req, res) => {
  try {
    const [workOrders, devices, healthScores] = await Promise.all([
      ...
      pool.query('SELECT device_id, overall_score, trend FROM device_health_scores ORDER BY calculated_at DESC LIMIT 25'),
    ]);
    const answer = await callAI([
      { role: 'system', content: 'You are a work order optimization engine for Vigilance AI. Analyze open work orders, device health trends, and priority levels to: re-prioritize based on risk impact, suggest batch groupings for efficiency, estimate urgency windows, identify work orders that could prevent cascading failures, and recommend optimal scheduling. Use markdown tables with priority rankings.' },
      { role: 'user', content: `Optimize work order scheduling:\n\nOPEN WORK ORDERS:\n${workOrders.rows.map(w => `[${w.priority}] ${w.work_order_number}: ${w.title} - ${w.device_type} "${w.device_name}" @ ${w.property_name} - Status: ${w.status} - Assigned: ${w.assigned_to}`).join('\n')}\n\nDEVICE STATUS:\n${devices.rows.filter(d => d.status !== 'Online').map(d => `${d.name} (${d.device_type}) @ ${d.property_name} - ${d.status}`).join('\n')}\n\nHEALTH SCORES:\n${healthScores.rows.map(h => `Device #${h.device_id}: Score ${h.overall_score}/100 - Trend: ${h.trend}`).join('\n')}` }
    ], 2500);
    res.json({ response: answer });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/ai/insider-threat', auth, async (req, res) => {
  try {
    const [access, credentials, incidents, visitors] = await Promise.all([
      ...
      pool.query('SELECT holder_name, badge_number, property_name, access_level, department, status FROM credentials'),
      pool.query("SELECT * FROM incidents WHERE incident_type IN ('Unauthorized Access', 'Tailgating', 'Policy Violation', 'Security Breach') ORDER BY id DESC LIMIT 20"),
      pool.query("SELECT visitor_name, host_name, property_name, purpose, photo_id_verified FROM visitors WHERE photo_id_verified = false OR status = 'Checked In'"),
    ]);
    const answer = await callAI([
      { role: 'system', content: 'You are an insider threat detection specialist for Vigilance AI. Analyze access patterns, credential anomalies, and incident history to identify potential insider threats. Look for: unusual access times, access to areas outside normal scope, suspended/expired credentials still being used, repeated denied access, tailgating patterns, and unverified visitors. Assign threat scores and recommend investigation priorities. Be analytical but avoid false accusations - present findings as risk indicators.' },
      { role: 'user', content: `Analyze for insider threat indicators:\n\nACCESS EVENTS:\n${access.rows.map(a => `${a.person_name} - ${a.direction} @ ${a.door_name}, ${a.property_name} - ${a.status} (${a.method}) - ${new Date(a.timestamp).toLocaleString()}`).join('\n')}\n\nCREDENTIALS:\n${credentials.rows.map(c => `${c.holder_name} - Badge: ${c.badge_number} - ${c.access_level} - Dept: ${c.department} - ${c.status}`).join('\n')}\n\nRELATED INCIDENTS:\n${incidents.rows.map(i => `[${i.severity}] ${i.title} @ ${i.property_name} - ${i.incident_type}`).join('\n')}\n\nUNVERIFIED/ACTIVE VISITORS:\n${visitors.rows.map(v => `${v.visitor_name} (Host: ${v.host_name}) @ ${v.property_name} - Purpose: ${v.purpose} - ID Verified: ${v.photo_id_verified}`).join('\n')}` }
    ], 2500);
    res.json({ response: answer });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/ai/energy-advisor', auth, async (req, res) => {
  try {
    const [devices, metrics, props, cameras] = await Promise.all([
      pool.query('SELECT name, device_type, property_name, status FROM devices'),
      pool.query('SELECT device_id, metric_type, avg_value FROM device_metrics WHERE metric_type IN (\'power_draw\', \'cpu_usage\', \'temperature\') ORDER BY recorded_at DESC LIMIT 100'),
      pool.query('SELECT name, sq_ft, floors, cameras_count, access_points FROM properties'),
      pool.query('SELECT property_name, status, recording_mode FROM cameras'),
    ]);
    const answer = await callAI([
      { role: 'system', content: 'You are an energy efficiency and sustainability advisor for Vigilance AI security infrastructure. Analyze device power consumption patterns, camera recording modes, device utilization, and property sizes to recommend energy-saving measures that don\'t compromise security. Estimate potential savings in kWh and cost. Recommend green initiatives like scheduled camera recording, device sleep modes, and optimal power management.' },
      { role: 'user', content: `Analyze energy usage and recommend sustainability improvements:\n\nPROPERTIES:\n${props.rows.map(p => `${p.name} - ${p.sq_ft} sqft, ${p.floors} floors, ${p.cameras_count} cameras, ${p.access_points} access points`).join('\n')}\n\nDEVICES:\n${devices.rows.map(d => `${d.name} (${d.device_type}) @ ${d.property_name} - ${d.status}`).join('\n')}\n\nPOWER METRICS:\n${metrics.rows.map(m => `Device #${m.device_id}: ${m.metric_type} = ${m.avg_value}`).join('\n')}\n\nCAMERA RECORDING:\n${cameras.rows.map(c => `${c.property_name}: ${c.status} - Mode: ${c.recording_mode}`).join('\n')}` }
    ], 2500);
    res.json({ response: answer });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/ai/executive-briefing', auth, async (req, res) => {
  try {
    const [props, incidents, cams, guards, devices, workOrders, tenants, healthScores] = await Promise.all([
      pool.query('SELECT * FROM properties'),
      pool.query('SELECT * FROM incidents ORDER BY id DESC LIMIT 20'),
      pool.query('SELECT property_name, status FROM cameras'),
      pool.query('SELECT property_name, status FROM guards'),
      pool.query('SELECT property_name, status FROM devices'),
      pool.query("SELECT priority, status FROM work_orders WHERE status != 'Completed'"),
      pool.query('SELECT company_name, property_name, employee_count FROM tenants'),
      pool.query('SELECT device_id, overall_score, trend FROM device_health_scores ORDER BY calculated_at DESC LIMIT 25'),
    ]);
    const answer = await callAI([
      { role: 'system', content: 'You are an executive briefing generator for Vigilance AI. Create a concise, C-level security summary suitable for board presentations. Include: portfolio overview, risk dashboard (use letter grades A-F), top 3 risks requiring attention, key metrics (uptime %, incident trends, compliance status), budget-impacting items, and strategic recommendations. Keep language non-technical and focused on business impact. Use professional markdown formatting.' },
      { role: 'user', content: `Generate executive security briefing:\n\nPORTFOLIO: ${props.rows.length} properties across ${[...new Set(props.rows.map(p => p.city))].join(', ')}\n\nINCIDENTS: ${incidents.rows.length} recent - ${incidents.rows.filter(i => i.severity === 'Critical').length} Critical, ${incidents.rows.filter(i => i.status !== 'Resolved').length} Open\n\nCAMERAS: ${cams.rows.length} total - ${cams.rows.filter(c => c.status === 'Online').length} online, ${cams.rows.filter(c => c.status === 'Offline').length} offline\n\nGUARDS: ${guards.rows.length} total - ${guards.rows.filter(g => g.status === 'On Duty').length} on duty\n\nDEVICES: ${devices.rows.length} total - ${devices.rows.filter(d => d.status === 'Online').length} online\n\nWORK ORDERS: ${workOrders.rows.length} open - ${workOrders.rows.filter(w => w.priority === 'Critical').length} critical priority\n\nTENANTS: ${tenants.rows.length} companies, ~${tenants.rows.reduce((s, t) => s + (t.employee_count || 0), 0)} employees\n\nHEALTH: Avg score ${Math.round(healthScores.rows.reduce((s, h) => s + h.overall_score, 0) / (healthScores.rows.length || 1))}/100, ${healthScores.rows.filter(h => h.trend === 'declining').length} declining devices` }
    ], 3000);
    res.json({ response: answer });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
