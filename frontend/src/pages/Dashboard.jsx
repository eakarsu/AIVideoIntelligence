      ...
      <div className="card-grid" style={{ marginBottom: 32 }}>
        ...
        <div className="module-card" style={{ cursor: 'default' }}>
          ...
          <div style={{ marginTop: 8 }}>
            {recentEvents.length === 0 && <div style={{ fontSize: 12, color: '#64748b' }}>No device events yet. Visit SOC Dashboard to start the simulator.</div>}
            {recentEvents.map((e, i) => (
              <div key={e.id || i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', fontSize: 12, borderBottom: i < recentEvents.length - 1 ? '1px solid #334155' : 'none' }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: e.severity === 'critical' ? '#ef4444' : e.severity === 'warning' ? '#f59e0b' : '#3b82f6', flexShrink: 0 }} />
                <span style={{ fontWeight: 600, color: '#e2e8f0' }}>{e.event_type?.replace(/_/g, ' ')}</span>
                <span style={{ color: '#64748b', marginLeft: 'auto', fontSize: 11 }}>{e.device_name}</span>
              </div>
            ))}
          </div>
        </div>
