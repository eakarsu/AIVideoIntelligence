...
// Feature: Camera health prediction forecasting device failures
...
export default function CfCameraHealthPredictionForecastingDeviceFailures() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-camera-health-prediction-forecasting-device-failures/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-camera-health-prediction-forecasting-device-failures', project: 'AIVideoIntelligence' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-camera-health-prediction-forecasting-device-failures/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Camera health prediction forecasting device failures</h1>
