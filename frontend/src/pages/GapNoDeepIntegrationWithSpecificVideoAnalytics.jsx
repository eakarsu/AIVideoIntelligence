...
export default function GapNoDeepIntegrationWithSpecificVideoAnalytics() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-deep-integration-with-specific-video-analytics-platforms/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-deep-integration-with-specific-video-analytics-platforms', project: 'AIVideoIntelligence' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-deep-integration-with-specific-video-analytics-platforms/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No deep integration with specific video analytics platforms (Axis, Hanwha) beyond generic integration layer</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_non_ai · AIVideoIntelligence</p>
      </div>
