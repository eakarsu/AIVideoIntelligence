...
async function request(url, options = {}) {
  ...
  if (!res.ok && res.status === 401) { localStorage.removeItem('vigilance_token'); window.location.href = '/login'; return null; }
  return res.json();
}

const STATUS_COLORS = { Online: '#107E3E', Offline: '#BB0000', Maintenance: '#E76500', Unregistered: '#6A6D70' };
const TYPE_ICONS = { Camera: '\u{1F4F9}', Reader: '\u{1F511}', Controller: '\u{1F5A5}\uFE0F', Sensor: '\u{1F4E1}', 'Alarm Panel': '\u{1F514}', 'Network Switch': '\u{1F500}' };
                  ...
                  <React.Fragment key={d.id}>
                    <tr onClick={() => selectDevice(d)}
                      ...
                      <td style={{ padding: '10px 14px', fontWeight: 600, color: '#1A2233' }}>
                        <span style={{ marginRight: 6 }}>{TYPE_ICONS[d.device_type] || '\u{1F4E6}'}</span>{d.name}
                      </td>
