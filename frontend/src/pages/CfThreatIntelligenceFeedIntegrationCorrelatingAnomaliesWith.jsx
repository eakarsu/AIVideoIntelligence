...
// Feature: Threat intelligence feed integration correlating anomalies with external IOCs
...
export default function CfThreatIntelligenceFeedIntegrationCorrelatingAnomaliesWith() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-threat-intelligence-feed-integration-correlating-anomalies-with-external/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-threat-intelligence-feed-integration-correlating-anomalies-with-external', project: 'AIVideoIntelligence' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-threat-intelligence-feed-integration-correlating-anomalies-with-external/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Threat intelligence feed integration correlating anomalies with external IOCs</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AIVideoIntelligence</p>
      </div>
