...
function authHeaders() {
  return { Authorization: `Bearer ${localStorage.getItem('vigilance_token')}` };
}
...
export default function FirmwareManagement() {
  ...
  useEffect(() => {
    const socket = io({ auth: { token: localStorage.getItem('vigilance_token') } });
  ...
  async function scheduleUpdate(device) {
    if (!scheduleDate) return;
    try {
      const res = await fetch('/api/firmware/schedule', {
        method: 'POST',
        headers: { ...authHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify({
          device_id: device.id,
      ...
      if (!res.ok) throw new Error('Failed to schedule update');
      setScheduling(null);
      setScheduleDate('');
      await fetchAll();
    } catch (err) {
  ...
  async function cancelUpdate(updateId) {
    try {
      const res = await fetch(`/api/firmware/updates/${updateId}/cancel`, {
        method: 'PUT',
        headers: authHeaders(),
      });
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, color: TEXT, padding: '24px 32px' }}>
      ...
      {tab === 'history' && (
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {updates.length === 0 && (
                <tr><td colSpan={8} style={{ padding: 40, textAlign: 'center', color: TEXT_DIM }}>
              ...
              {updates.map(u => {
                ...
                return (
                  <tr key={u.id} onClick={() => setSelectedUpdate(u)}
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      {(u.status === 'Pending' || u.status === 'Scheduled') && (
                        <button onClick={(e) => { e.stopPropagation(); cancelUpdate(u.id); }}
      ...
      {/* Schedule Modal */}
      {scheduling && (
        <div onClick={() => { setScheduling(null); setScheduleDate(''); }} style={{
          position: 'fixed', top: 0, left: 0, right: 0, bottom: 0,
          background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(6px)',
          ...
          <div onClick={e => e.stopPropagation()} style={{
            background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16,
            width: 420, padding: 28, boxShadow: '0 32px 80px rgba(0,0,0,0.5)',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
              <FaClock style={{ color: YELLOW }} />
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Schedule Firmware Update</h3>
            </div>
            <div style={{ fontSize: 14, color: TEXT_DIM, marginBottom: 16 }}>
              {scheduling.device_name} &mdash; {scheduling.current_version} &#8594; {scheduling.latest_version}
            </div>
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 12, color: TEXT_DIM, display: 'block', marginBottom: 6 }}>Schedule Date & Time</label>
              <input type="datetime-local" value={scheduleDate} onChange={e => setScheduleDate(e.target.value)}
                style={{ width: '100%', padding: '10px 14px', background: BG, border: `1px solid ${BORDER}`,
                  borderRadius: 8, color: TEXT, fontSize: 14 }} />
            </div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button onClick={() => { setScheduling(null); setScheduleDate(''); }}
                style={{ padding: '8px 18px', background: BORDER, color: TEXT, border: 'none',
                  borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>Cancel</button>
              <button onClick={() => scheduleUpdate(scheduling)} disabled={!scheduleDate}
                style={{ padding: '8px 18px', background: scheduleDate ? YELLOW : BORDER,
                  color: scheduleDate ? '#000' : TEXT_DIM, border: 'none', borderRadius: 8,
                  cursor: scheduleDate ? 'pointer' : 'default', fontSize: 13, fontWeight: 600 }}>
                Schedule Update
              </button>
            </div>
          </div>
