...
export default function GapNoVisionVideoFrameMlAnalysisFocused() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-vision-video-frame-ml-analysis-focused-on/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-vision-video-frame-ml-analysis-focused-on', project: 'AIVideoIntelligence' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-vision-video-frame-ml-analysis-focused-on/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No vision/video frame ML analysis (focused on metadata, not pixels)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AIVideoIntelligence</p>
      </div>
