...
export default function SOCDashboard() {
  ...
  useEffect(() => {
    let socket;
    try {
      socket = io('http://localhost:4000', { transports: ['websocket', 'polling'], reconnectionAttempts: 5 });
  ...
  const runAnalysis = async () => {
    ...
    try {
      const token = localStorage.getItem('vigilance_token');
      if (!token) { setAnalysis('DEBUG: No vigilance_token found in localStorage'); setAnalyzing(false); return; }
