...
export default function GapAiCoverageIsComprehensiveForTheDomain() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-ai-coverage-is-comprehensive-for-the-domain/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-ai-coverage-is-comprehensive-for-the-domain', project: 'AIVideoIntelligence' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-ai-coverage-is-comprehensive-for-the-domain/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">AI coverage is comprehensive for the domain</h1>
