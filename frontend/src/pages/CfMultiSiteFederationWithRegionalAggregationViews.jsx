...
// Feature: Multi-site federation with regional aggregation views
...
export default function CfMultiSiteFederationWithRegionalAggregationViews() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-site-federation-with-regional-aggregation-views/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-multi-site-federation-with-regional-aggregation-views', project: 'AIVideoIntelligence' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-site-federation-with-regional-aggregation-views/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Multi-site federation with regional aggregation views</h1>
